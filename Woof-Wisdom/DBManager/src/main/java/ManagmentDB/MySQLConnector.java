...
public class MySQLConnector {
    ...
    private static final String SSH_HOST = "ec2-16-170-201-200.eu-north-1.compute.amazonaws.com";
...
//            }
    ...
    public static void incrementCommentCountForQuestion(int questionId) {
        try (Connection conn = getConnection()) {
            ...
            String sql = "UPDATE forums SET comment_num = comment_num + 1 WHERE questionID = ?";
            PreparedStatement stmt = conn.prepareStatement(sql);
    ...
    public static Map<Integer, Integer> getCommentCountsForAllQuestions() {
        ...
        try (Connection conn = getConnection()) {
            String sql = "SELECT questionID, COUNT(*) as comment_num FROM comments GROUP BY questionID";  // Replace 'comments' and 'question_id' with actual table and column names.
            PreparedStatement stmt = conn.prepareStatement(sql);

            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    int questionId = rs.getInt("questionID");
                    int commentCount = rs.getInt("comment_num");
